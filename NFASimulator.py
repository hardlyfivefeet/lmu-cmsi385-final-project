...
def add_transition(transition_function, state, action, new_state):
    ...
    if (len(sys.argv) <= 1):
        ...
        for line in description_lines:
            if ":" in line:
                ...
                add_transition(transition_function, state, action, new_state)
            if "->" in line and ":" not in line:
                state, new_state = line.split("->")
                all_states.add(state)
                all_states.add(new_state)
                add_transition(transition_function, state, "", new_state)
            if "START" in line:
